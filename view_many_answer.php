...
<div class="index">
  ...
  <div class="auto-group-k445-dpy">
    <?php
        ...
        ?>
    <h1>DẠNG CÂU HỎI: Nhiều đáp án</h1>
    ...
    <div style="font-size: 15px; margin: 0px 10%;" >
       ...
       <?php
                    ...
                    if(mysqli_num_rows($do)>0){
                        while($row=mysqli_fetch_array($do)) {   
                            if($row['id_question']==$_SESSION['view']){
                                ...
                                $count=0;
                                $answer_correct=explode("/",$row['answer_correct']);
                                ...
                                for ($i=1; $i <=$count ; $i++) { 
                                    ...
                                    foreach($answer_correct as $key_correct){
                                        if($key_correct==$answer_array[$i-1]){
                                            $_POST[$i]="checked"; //Kiểm tra nếu đúng thì tích checkbox;
                                        }
                                    }
                                }
                            }
                ...
                function question_answer(){
                    ...
                    echo '<div class="input-group-text"><input type="checkbox" id="' . $i . '" name="' . $i . '" value="' . $i . '" ' . $isChecked . ' readonly></div>';
                                    ?>
